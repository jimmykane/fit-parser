 ...
 *
 * Message, field, and type definitions come from fit-file-parser's handwritten
 * MIT-licensed table immediately before profile code generation was added.
 * The Garmin product map is restored from the earlier pre-SDK repository
 * boundary. See PROFILE.md for the immutable source commits and update policy.
 */
...
const HISTORICAL_PROFILE_MESSAGES = {
  0: {
    name: 'file_id',
...
} as unknown as Record<number, Message>

const HISTORICAL_PROFILE_TYPES = {
  file: {
    1: 'device',
    2: 'settings',
    3: 'sport',
...
} as Record<string, Record<number, string | number>>
...
function field(
  ...
  units = '',
  options: { array?: boolean, baseType?: string, offset?: number } = {},
): MessageObject {
  return {
    field: name,
    type,
    ...(options.baseType ? { baseType: options.baseType } : {}),
...
const PROFILE_MESSAGE_CORRECTIONS: Record<number, Message> = {
  6: {
    name: 'bike_profile',
    19: field('crank_length', 'uint8', 2, 'mm', { offset: 110 }),
  },
  18: {
    name: 'session',
    139: field('avg_vam', 'uint16', 1000, 'm/s'),
    140: field('avg_depth', 'uint32', 1000, 'm'),
    168: field('training_load_peak', 'sint32', 65536),
    193: field('workout_rpe', 'uint8', 1),
    196: field('metabolic_calories', 'uint16', 1, 'kcal'),
  },
  19: {
    name: 'lap',
    149: field('total_grit', 'float32', 1, 'kGrit'),
    150: field('total_flow', 'float32', 1, 'Flow'),
    153: field('avg_grit', 'float32', 1, 'kGrit'),
    154: field('avg_flow', 'float32', 1, 'Flow'),
  },
  20: {
    name: 'record',
    17: field('speed1s', 'uint8', 16, 'm/s', { array: true }),
    92: field('depth', 'uint32', 1000, 'm'),
    93: field('next_stop_depth', 'uint32', 1000, 'm'),
    123: field('air_time_remaining', 'uint32', 1, 's'),
    124: field('pressure_sac', 'uint16', 100, 'bar/min'),
    125: field('volume_sac', 'uint16', 100, 'L/min'),
    126: field('rmv', 'uint16', 100, 'L/min'),
    127: field('ascent_rate', 'sint32', 1000, 'm/s'),
    129: field('po2', 'uint8', 100, 'percent'),
  },
  55: {
    name: 'monitoring',
    28: field('intensity', 'uint8', 10),
    31: field('ascent', 'uint32', 1000, 'm'),
    32: field('descent', 'uint32', 1000, 'm'),
  },
  103: {
    name: 'monitoring_info',
    3: field('cycles_to_distance', 'uint16', 5000, 'm/cycle', { array: true }),
    4: field('cycles_to_calories', 'uint16', 5000, 'kcal/cycle', { array: true }),
  },
  142: {
    name: 'segment_lap',
    84: field('total_grit', 'float32', 1, 'kGrit'),
    85: field('total_flow', 'float32', 1, 'Flow'),
    86: field('avg_grit', 'float32', 1, 'kGrit'),
    87: field('avg_flow', 'float32', 1, 'Flow'),
  },
  188: {
    name: 'ohr_settings',
    0: field('enabled', 'switch', 1),
  },
  206: {
    name: 'field_description',
    2: field('fit_base_type_id', 'fit_base_type', 1),
  },
  211: {
    name: 'monitoring_hr_data',
    0: field('resting_heart_rate', 'uint8', 1, 'bpm'),
    1: field('current_day_resting_heart_rate', 'uint8', 1, 'bpm'),
  },
  216: {
    name: 'time_in_zone',
    0: field('reference_mesg', 'mesg_num', 1),
    1: field('reference_index', 'message_index', 1),
  },
  259: {
    name: 'dive_gas',
    3: field('mode', 'dive_gas_mode', 1),
  },
  262: {
    name: 'dive_alarm',
    0: field('depth', 'uint32', 1000, 'm'),
  },
  268: {
    name: 'dive_summary',
    2: field('avg_depth', 'uint32', 1000, 'm'),
    3: field('max_depth', 'uint32', 1000, 'm'),
    11: field('bottom_time', 'uint32', 1000, 's'),
    12: field('avg_pressure_sac', 'uint16', 100, 'bar/min'),
    13: field('avg_volume_sac', 'uint16', 100, 'L/min'),
    14: field('avg_rmv', 'uint16', 100, 'L/min'),
    15: field('descent_time', 'uint32', 1000, 's'),
    16: field('ascent_time', 'uint32', 1000, 's'),
    17: field('avg_ascent_rate', 'sint32', 1000, 'm/s'),
    22: field('avg_descent_rate', 'uint32', 1000, 'm/s'),
    23: field('max_ascent_rate', 'uint32', 1000, 'm/s'),
    24: field('max_descent_rate', 'uint32', 1000, 'm/s'),
    25: field('hang_time', 'uint32', 1000, 's'),
  },
  275: {
    name: 'sleep_level',
    0: field('sleep_level', 'sleep_level', 1),
  },
  319: {
    name: 'tank_update',
    1: field('pressure', 'uint16', 100, 'bar'),
  },
  323: {
    name: 'tank_summary',
    1: field('start_pressure', 'uint16', 100, 'bar'),
    2: field('end_pressure', 'uint16', 100, 'bar'),
    3: field('volume_used', 'uint32', 100, 'L'),
    253: field('timestamp', 'date_time', 1, 's'),
  },
}

const PROFILE_TYPE_CORRECTIONS: Record<string, Record<number, string | number>> = {
  dive_gas_mode: {
    0: 'open_circuit',
...
export const PROFILE_MESSAGES = mergeMessages()
export const PROFILE_TYPES = mergeTypes()
