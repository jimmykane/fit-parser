...
3.  **Identify Message ID**:
    - Check `src/profile.ts` for the maintained message definition.
    - Inspect `unmapped_messages` for the global message number, field number,
      base type, and exact bytes recorded from the observed file.
4.  **Fix**:
    - Update `src/profile.ts` only when the mapping has a documented,
      maintainable source.
